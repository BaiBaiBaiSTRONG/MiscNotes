...
###### 8.4.5.2第二次机会算法Second Chance Algorithm

又叫做时钟算法（如果表中的所有引用位都已设置,则降级为FIFO）

###### 8.4.5.3增强型机会算法Enhanced Second Chance Algorithm(一个引用位，一个修改位)

( 0, 0 ) - 最近既没使用也没修改过。( 0, 1 ) - 最近未使用,但已修改。( 1, 0 ) - 最近使用,但未修改。( 1, 1 ) - 最近使用和修改过
如果可能,首选替换干净的页面。最先选择（0，0）
...
![在这里插入图片描述](OperatingSystemReview.assets/20190617154145159.png)


##### ==8.6.3　缺页错误频率 273==

页面故障率与工作集之间存在直接关系,因为进程从一个局部地移动到另一个局部，因而长度量的选择对于工作集模型的成功至关重要。
![在这里插入图片描述](OperatingSystemReview.assets/20190617154331518.png)
...
计算机访问磁盘有两种方式。一种方式是通过I/O端口，称为**主机连接存储**， 另外一种是分布式系统的远程主机（称为网络连接存储）。磁盘驱动器可以直接连接到特定主机(本地磁盘)或网络。

##### 9.3.1　主机连接存储 300

主机连接存储：主机连接通过本地的端口来访问存储。这些端口使用多种技术。典型的台式机采用I/O 总线，一般**SATA的接口**更为常用。主机链接存储中本地磁盘通过 I/O 端口访问。
...
##### 9.4.2　最短寻道时间优先SSTF调度 （最短寻找时间优先，可能导致饥饿）302
...
操作系统所需的交换空间量因使用方式而异。有的需要很多，有的一点也不需要。
交换空间可以物理地位于两个位置之一:原始分区（更快更高效），作为常规文件系统一部分的大型交换文件（易于实现，不高效）。
